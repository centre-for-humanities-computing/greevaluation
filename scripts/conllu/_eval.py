"""Script from UniversalDependencies for evaluating CONLL-U files"""
...
def process_enhanced_deps(deps):
    ...
    if deps != "" and deps != "_":
        for edep in deps.split("|"):
            ...
            steps = path.split(
                ">"
            )  # collapsing empty nodes gives rise to paths like this : 3:conj:en>obl:voor
...
def load_conllu(file, treebank_type):
    ...
    while True:
        ...
        if not line:
            ...
            def process_word(word):
                if word.parent == "remapping":
                    raise UDError(
                        "There is a cycle in the sentence that ends at line %d"
                        % line_idx
                    )
                if word.parent is None:
                    ...
                    if head < 0 or head > len(ud.words) - sentence_start:
                        raise UDError(
                            "HEAD '{}' points outside of the sentence that ends at line {}".format(
            ...
            for word in ud.words[sentence_start:]:
                ...
                processed_deps = []
                for head, steps in word.columns[
                    DEPS
                ]:  # (3,['conj:en','obj:voor'])
                    ...
                    for hd, steps in enhanced_deps:
                        ...
                        for dep in steps:
                            ...
                            if depparts[0] in CASE_DEPRELS:
                                if len(depparts) == 2 and not (
                                    depparts[1] in UNIVERSAL_DEPREL_EXTENSIONS
                                ):
                                    dep = depparts[0]
            ...
            if len(ud.words) == sentence_start:
                raise UDError(
                    "There is a sentence with 0 tokens (possibly a double blank line) at line %d"
                    % line_idx
                )
            ...
            if (
                ...
                raise UDError(
                    "There are no roots in the sentence that ends at %d"
                    % line_idx
                )
            if not treebank_type.get("multiple_roots_okay", False):
                if (
                    ...
                    raise UDError(
                        "There are multiple roots in the sentence that ends at %d"
                        % line_idx
                    )
        ...
        if len(columns) != 10:
            raise UDError(
                "The CoNLL-U line does not contain 10 tab-separated columns at line {}: '{}'".format(
                    line_idx, _encode(line)
                )
            )
        ...
        if "." in columns[ID]:
            ...
            if treebank_type.get("no_empty_nodes", False):
                raise UDError(
                    "The collapsed CoNLL-U line still contains empty nodes at line {}: {}".format(
                        line_idx, _encode(line)
                    )
                )
            else:
        ...
        if not columns[FORM]:
            raise UDError(
                "There is an empty FORM in the CoNLL-U file at line %d"
                % line_idx
            )
        ...
        if "-" in columns[ID]:
            try:
                ...
                raise UDError(
                    "Cannot parse multi-word token ID '{}' at line {}".format(
                        _encode(columns[ID]), line_idx
                    )
                )
        ...
        # Basic tokens/words
            ...
            try:
                ...
                raise UDError(
                    "Cannot parse word ID '{}' at line {}".format(
                        _encode(columns[ID]), line_idx
                    )
                )
            if word_id != len(ud.words) - sentence_start + 1:
                raise UDError(
                    "Incorrect word ID '{}' for word '{}', expected '{}' at line {}".format(
                        _encode(columns[ID]),
            ...
            try:
                ...
                raise UDError(
                    "Cannot parse HEAD '{}' at line {}".format(
                        _encode(columns[HEAD]), line_idx
                    )
                ) from e
            if head_id < 0:
            ...
            for parent, dep in gold_deps:
                ...
                for sparent, sdep in system_deps:
                    ...
                    if dep == sdep or (eulas_dep == eulas_sdep and EULAS):
                        if parent == alignment.matched_words_map.get(
                            sparent, "NotAligned"
                        ):
                            correct += 1
                        elif (
                            parent == 0 and sparent == 0
                        ):  # cases where parent is root
                            correct += 1
...
def evaluate_wrapper(args):
    ...
    treebank_type["no_shared_parents_in_coordination"] = (
        1 if "2" in enhancements else 0
    )
    treebank_type["no_shared_dependents_in_coordination"] = (
        1 if "3" in enhancements else 0
    )
    treebank_type["no_control"] = 1 if "4" in enhancements else 0
    treebank_type["no_external_arguments_of_relative_clauses"] = (
        1 if "5" in enhancements else 0
    )
...
def build_evaluation_table(evaluation, verbose, counts, enhanced):
    ...
    if not verbose and not counts:
        ...
        if enhanced:
            text.append(
                "ELAS F1 Score: {:.2f}".format(100 * evaluation["ELAS"].f1)
            )
            text.append(
                "EULAS F1 Score: {:.2f}".format(100 * evaluation["EULAS"].f1)
            )
    else:
        if counts:
            text.append(
...
def main():
    ...
    results = build_evaluation_table(
        evaluation, args.verbose, args.counts, args.enhanced
    )
    print(results)
