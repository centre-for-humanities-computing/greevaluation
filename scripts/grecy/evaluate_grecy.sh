...
for CORPUS in "joint" "perseus" "proiel"
    do
    mkdir -p "metrics/$MODEL1/1.0.0/"
    spacy evaluate "$MODEL1" corpus/binary/$CORPUS.spacy --output "metrics/$MODEL1/1.0.0/$CORPUS.json"
done
...
for CORPUS in "joint" "perseus" "proiel"
    do
    mkdir -p "metrics/$MODEL2/1.0.0/"
    spacy evaluate "$MODEL2" corpus/binary/$CORPUS.spacy --output "metrics/$MODEL2/1.0.0/$CORPUS.json"
done
